...
切需求开展本文的研究，重点针对碳交易价格的影响因素及预测问题作出具体分析。
...
在拟合同时包含趋势性、周期性的时间序列时存在明显不足，prophet预测模型能够对有以
上特征的时间序列做出很好的拟合，并且模型参数直观易解释，在预测时可以快速识别预测
误差较大的部分，并做出相应调整。下面对prophet模型作出详细介绍。

prophet预测模型包含三个成分，分别是趋势、季节性、假期。

$$y{(t)}=g{(t)}+s{(t)}+h{(t)}+\varepsilon_t$$ {#eq:model-prophet}

式中, $g{(t)}$ 用来对时间序列的非周期性即趋势性进行建模，$s{(t)}$表示周期性变化，
如常见的周和季节性，$h{(t)}$ 表示假日效应，每个假日的时间在较短的时间范围内变动，
 $\varepsilon_t$ 表示服从正态分布的误差项，其包含了其余不可控因素。
 
1. 趋势模型（内容包含分段线性回归和逻辑回归的介绍、突变点的生成、在预测时的条件假定）

对于时间趋势的拟合，prophet模型有两种形式，一种是非线性的 $logistic$ 增长模型，一种
是分段线性模型。
...
与式 [@eq:model24] 不同的是，我们假定容载量 $C(t)$ 不是参数，在模型构建时，需要结合实际
情况设定。

在趋势模型的预测部分，通常通过如下方式定义突变点
...
并且假定未来突变点的平均频率和平均调整幅度与历史数据保持一致，$\lambda$ 的设定将根据历史
数据的推断给出。
 
2. 季节性（季节部分即产生傅里叶序列，说明N的选择）

时间序列常呈现出一定的周期性，用考虑了季节效应的模型做预测对提高预测准确性起到很大作用。下
面对prophet预测模型的季节效应做简单介绍。
...
大量实践证明，当以年或者周为周期时，分别设定 $N=10$  ， $N=3$ 使得模型拟合效果较佳。
...
由于假日所在日期排列不具有周期性（如元旦、五一、端午节等假期），所以不能包括在季节效应中。
prpphet预测对此的处理是通过设置假日可能存在的日期区间来完成的，由于本文所用的碳交易价格序列
不包括假日数据，所以在此不做过多描述。
...
```{r fig2, out.width="100%", echo=F,fig.cap="ARMA模型预测效果图",dev="png",results='markup', cache=F}
arma <- read.csv("./data/arima_result.csv",header = T,sep=",")
par(mar=c(5,5,2,2)+0.1,oma=c(0,0,0,0))
plot(arma$x,lty=1,type="b",pch=16,xlab="日期",ylab="碳价/元",xaxt="n",ylim = c(11,19),cex.lab=.8,cex.axis=.7)
...
axis(1,at=c(1,50,100,150,200,250,300,350),labels = c("2017/1/4","2017/3/23","2017/6/9","2017/8/21","2017/11/6",
                                                     "2018/1/7","2018/4/9","2018/6/21"),cex.axis=.7)
legend("topright",c("预测值","真实值"),pch=c(16,4),lty=c(1,2),bty="n",cex=0.7)
```

### Prophet模型下的碳交易价格预测

本文首先对碳交易价格序列的特征进行判断，显示其不仅包含不同时间段的趋势变化，时间
序列还呈现一定的周期性，对此，接下来选用R语言中的prophet预测模型对碳交易价格序列
进行拟合，通过误差分析可以对模型质量进行判断，进而调整模型参数，最后利用所得最优
模型对碳交易价格序列做10期预测。
...
plot(history$y,type="l",pch=20,xlab="日期",ylab="碳价/元",xaxt="n",cex.lab=0.8,cex.axis=0.7)
