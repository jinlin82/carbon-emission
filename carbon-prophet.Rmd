...
通过将历史数据中的趋势特点运用至未来预测，并且假定未来突变点的平均调整频率和平均调整幅度与历史数
...
该期间具有相同的假日效应。具体设置通过参数 lower_window 和 upper_window 实现。lower_window是负数，表示
某假日之前的天数，upper_window是正数，表示某假日之后的天数。如铁路运输量在清明节前后就会出现波动，而不
只是在清明节当天，故可以设置lower_window=-1，upper_window=2。另外，还需设置示性函数和参数 $k_i$ ,示性
...
tab7 <- read.csv('./data/updata/holidays.csv')
knitr::kable(tab7, row.names =F, align = "l", caption="holidays",
             digits=2,
...
m <- add_seasonality(m, name='working', period=5, fourier.order=3)
m <- fit.prophet(m, history2)
